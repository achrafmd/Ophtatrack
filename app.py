...
def ensure_media_ws(sh):
    """Ensure 'Media' worksheet exists with correct headers."""
    try:
        ws = sh.worksheet(MEDIA_SHEET)
        # if exists but empty, ensure headers:
...
def append_dict(ws, d: dict):
    ...
    # Create headers if somehow missing
    if not header:
        header = list(d.keys())
        ws.update([header])
...
def add_patient_form(data):
    ...
    if "Pathologie / Catégorie" in (menu_df.columns if not menu_df.empty else []):
    ...
    with st.form("add_form_full"):
        ...
        with c2:
            ...
            dictated = voice_dictation("notes_text")
        ...
        if ok:
            ...
            if img is not None and data["mode"]=="sheets":
                ws_med = ensure_media_ws(data["sh"])
                content = img.read()
...
def page_patients(data):
    ...
    with c2:
        menu_patho_col = "Pathologie / Catégorie" if "Pathologie / Catégorie" in (menu_df.columns if not menu_df.empty else []) else None
...
def main():
    ...
    st.title(APP_TITLE)
    st.caption("Version complète + dictée vocale. Correction auto de l’onglet 'Media'.")
