...
LOCAL_EXCEL = "OphtaTrack_Template.xlsx"  # fallback si pas de Sheets
MEDIA_SHEET = "Media"
...
def load_data():
    ...
    if sh:
        try:
            ...
            st.error("Assure-toi que les onglets 'Menu', 'Patients', 'Paramètres', 'Statistiques', 'Media' existent.")
            st.stop()
    ...
    if not os.path.exists(LOCAL_EXCEL):
        return {"mode":"local","sh":None,
                "menu": pd.DataFrame(), "patients": pd.DataFrame(),
                "params": pd.DataFrame(), "stats": pd.DataFrame(),
                "media": pd.DataFrame()}
...
def voice_dictation(key: str, label="🎙️ Dictée vocale"):
    """
    Renders a small widget that uses the browser Web Speech API to fill a hidden textarea.
    The recognized text is returned in st.session_state[key]. Works on iOS Safari (with mic permission).
    """
    ...
    <div>
      ...
      <script>
        ...
        if (!supportsSpeech()) {{
            ...
            rec.onstart = ()=>{{ statusEl.textContent = "Écoute… Parle."; }};
    ...
    # Retrieve value possibly set by JS
...
def add_patient_form(data):
    ...
    if "Pathologie / Catégorie" in menu_df.columns:
    ...
    with st.form("add_form_full"):
        ...
        with c2:
            ...
            dictated = voice_dictation("notes_text")  # Web Speech API
        ...
        if ok:
            ...
            if data["mode"]=="sheets":
                ws_pat = data["sh"].worksheet("Patients")
                append_dict(ws_pat, new_row)
                st.success("✅ Enregistré de façon PERSISTANTE (Google Sheets).")
                st.cache_data.clear()
            else:
                st.warning("Mode local : configure Google Sheets pour la persistance.")
...
def page_patients(data):
    ...
    with c2:
        menu_patho_col = "Pathologie / Catégorie" if "Pathologie / Catégorie" in menu_df.columns else (menu_df.columns[0] if not menu_df.empty else None)
...
def main():
    ...
    st.title(APP_TITLE)
    st.caption("Version complète + dictée vocale (Web Speech API).")
